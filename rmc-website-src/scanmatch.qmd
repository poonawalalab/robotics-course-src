...
Given scan points $\left\{(x_j, y_j)\right\}$ in the robot frame at pose $(X_i, Y_i, \theta_i)$ in the current frame $F$:

::: {.callout-tip icon=false appearance="minimal"}
$$
\begin{aligned}
x_{F} &= X_i + x_j \cos \theta_i - y_j \sin \theta_i \\
y_{F} &= Y_i + x_j \sin \theta_i + y_j \cos \theta_i
\end{aligned}
$$
:::

::: {.fragment fragment-index=0}
This transformation transforms all points in from the robot frame to the frame $F$
:::
...
2. Find $(X_i, Y_i, \theta_i)$ that transforms the second scan to align with the reference
...
Estimate the sequence of poses $\{(X_i, Y_i, \theta_i)\}$ from a sequence of scans

::: {.fragment fragment-index=0}
**Three components needed:**

1. **Evaluation function**: Scalar measure of alignment quality
2. **Optimization algorithm**: Minimize the evaluation function
3. **Path integration**: Convert relative transformations to global trajectory
:::
...
- Output: Sequence of poses $(X_i, Y_i, \theta_i)$
  - 2D plot of path $(X_i, Y_i)$
  - Lines indicating orientation $\theta_i$

## Evaluation Function

How "good" is a candidate transformation $(X, Y, \theta)$?

::: {.fragment fragment-index=0}
Common approach: Sum of squared distances between transformed points and reference
$$
L(X, Y, \theta) = \sum_j d_j^2
$$
:::

::: {.fragment fragment-index=1}
**Challenge**: The same physical point is mostly *not* observed in both scans
:::

## Optimization Methods

Given two scans and an evaluation function:

- Use optimization algorithms to find $(X, Y, \theta)$ that minimizes the cost
- Methods from nonlinear least squares apply here
...
Scan matching gives **relative** transformations between consecutive scans
...
## Report Requirements {.smaller}

The evaluation is based on a report containing:

1. Description of all evaluation functions and algorithms considered
2. Discussion of changes made along the way and **why**
3. Whether changes had the expected impact or not
4. Performance summary:
   - Average run time per iteration
   - Number of iterations per scan match
   - Average error in scan match
...
::: {.callout-warning}
It is **not expected** that you will obtain highly accurate predictions of the robot's motion
:::

::: {.fragment fragment-index=0}
**Emphasis is on connecting theory and practice:**

- Try simple things first
- Use results (and surprises) to inform choices
- An approach that produces accurate predictions without explainable choices is not success
:::
...
- Log all decisions and save plots as you go
- Create reusable visualization tools early
- Don't only copy existing ICP/NDT implementations
  - Understanding the choices matters more than accuracy

::: {.callout-note}
A well-documented journey with modest results beats an unexplained accurate solution
:::
...
5. **Path integration**: Chain relative poses into trajectory
