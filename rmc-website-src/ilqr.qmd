...
$$\hat{q}_x = q_t + A_{t}^T \mathbf{v}_{t+1}, \qquad \hat{q}_u = r_t + B_{t}^T \mathbf{v}_{t+1}$$
$$\hat{Q}_{xx} = Q_t^{cost} + A_{t}^T \mathbf{V}_{t+1} A_{t}, \quad \hat{Q}_{uu} = R_t + B_{t}^T \mathbf{V}_{t+1} B_{t}, \quad \hat{Q}_{ux} = S_t + B_{t}^T \mathbf{V}_{t+1} A_{t}$$

## Backward Pass: Optimal Gains {.smaller}

Minimize $\tilde{Q}_{t}$ over $\delta u$ (holding $\delta x$ fixed):
$$\frac{\partial \tilde{Q}_{t}}{\partial \,\delta u} = \hat{q}_u + \hat{Q}_{uu}\,\delta u + \hat{Q}_{ux}\,\delta x = 0$$
...
Starting from $\hat{x}_0 = \bar{x}_0$[^11], for $t = 0, 1, \dots, T-1$:
...
[^11]: $\hat{x}$ is the updated nominal value (originally $\bar{x}$) of $x$
...
| Term | Character | Role |
|------|-----------|------|
| $k_{t} = -\hat{Q}_{uu}^{-1} \hat{q}_u$ | **Open-loop** (feedforward) | Depends only on $t$; the search direction |
| $K_{t} \delta x_{t}$ | **Closed-loop** (feedback) | Reacts to whatever $\delta x_{t}$ the nonlinear rollout produces |

::: {.callout-note appearance="minimal"}
**Connection to LQR:** In LQR the full optimal policy is $u_t^* = -K_t x_t$ — the feedback gain IS the solution, not a correction. iLQR adds the feedforward $k_t$ only because $\hat{q}_u \neq 0$ off the optimum; at convergence $k_t \to 0$ and only the feedback remains.
:::
...
At convergence, $k_{t} = -\hat{Q}_{uu}^{-1} \hat{q}_u \to 0$, which means:

$$\hat{q}_u = r_t + B_{t}^T \mathbf{v}_{t+1} = 0 \quad \text{for all } t$$
...
$$\frac{\partial c_{t}}{\partial u_{t}} + B_{t}^T \lambda_{t+1} = 0, \qquad \lambda_{t+1} = \mathbf{v}_{t+1}$$
...
**Convergence ↔ optimality:** $\hat{q}_u = 0$ is the discrete-time PMP stationarity condition
