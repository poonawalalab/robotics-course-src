...
- Only $u_t^*$ is ever applied — the rest of the plan is discarded and recomputed
...
Without non-dynamics constraints **LQR and MPC give the identical control** (finite horizon case)
:::
...
**Claim:** With terminal cost $V_f$ satisfying $V_f(Ax + BKx) \leq V_f(x) - c(x, Kx)$ for $x \in \mathcal{X}_f$, the MPC value function $J^*(x_t)$ is a **Lyapunov function**.

::: {.fragment fragment-index=1}
**Key inequality** (shifting argument):

At time $t$: optimal cost is $J^*(x_t) = c(x_t, u_0^*) + \cdots + V_f(x_T^*)$

At time $t{+}1$: use the *suboptimal* shifted sequence $u_1^*, \dots, u_{T-1}^*, K x_T^*$.
Its cost is at most:

$$J^*(x_{t+1}) \leq J^*(x_t) - c(x_t, u_0^*)$$
:::

::: {.fragment fragment-index=2}
Since $c(x_t, u_t^*) \geq 0$ with equality only at $x_t = 0$:

$$J^*(x_{t+1}) \leq J^*(x_t) - c(x_t, u_0^*) < J^*(x_t) \quad \text{for } x_t \neq 0$$
...
::: {.fragment fragment-index=1}
**Shift and hold:**
$$\bar{u}_{t+1} = [u_1^*, u_2^*, \dots, u_{T-1}^*, u_{T-1}^*]$$
